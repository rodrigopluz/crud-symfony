...
class ProdutoController extends Controller
{
    ...
    public function editarProdutoAction($slug)
    {
        if (isset($slug)) {
            $pu = new PersistenceUnity('AppBundle:Produto', $this);
            $produto = $pu->findBy(['slug' => $slug]);
            if ($produto != null) {
                $form = $this->createEditForm($produto);
                return $this->render('produto/form.html.twig', ['produto' => $produto, 'form' => $form->createView()]);
            }
    ...
    public function excluirProdutoAction(Request $request) 
    {
        ...
        if (isset($id)) {
            ...
            if ($produto != null) {
                ...
                $item = $puItem->findBy(['produto' => $produto]);
                if ($item != null) {
    ...
    {
        return $this->createForm(new ProdutoType(), $entity, [
    ...
    private function createEditForm(Produto $entity)
    {
        return $this->createForm(new ProdutoType(),
            $entity,
            [
                'action' => $this->generateUrl('atualizar_produto', ['id' => $entity->getId()]),
                'method' => 'POST'
            ]);
    }
}