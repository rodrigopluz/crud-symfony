...
class PessoaController extends Controller
{
    ...
    public function editarClienteAction($slug)
    {
        if (isset($slug)) {
            $pu = new PersistenceUnity('AppBundle:Pessoa', $this);
            $cliente = $pu->findBy(['slug' => $slug]);
            if ($cliente != null) {
                $form = $this->createEditForm($cliente);
                return $this->render('cliente/form.html.twig', ['cliente' => $cliente, 'form' => $form->createView()]);
            }
    ...
    public function excluirClienteAction(Request $request)
    {
        ...
        if (isset($id)) {
            ...
            if ($cliente != null) {
                ...
                $item = $puItem->findBy(['cliente' => $cliente]);
    ...
    {
        return $this->createForm(new PessoaType(), $entity, [
    ...
    private function createEditForm(Pessoa $entity)
    {
        return $this->createForm(new PessoaType(),
            $entity,
            [
                'action' => $this->generateUrl('atualizar_cliente', ['id' => $entity->getId()]),
                'method' => 'POST'
            ]);
    }
}